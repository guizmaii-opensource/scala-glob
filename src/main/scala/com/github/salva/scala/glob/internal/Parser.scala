...
object Parser {
  ...
  def literal[_: P]: P[Token] = P((regularChars | escapedChar).rep(1)./.map(a => Literal(a.reduce(_ + _))))

  def special[_: P](s: String): P[Special] = P(P(s)./.map(_ => Special(s)))
  ...
  def commaSepTokens[_: P, A]: P[Seq[Seq[Token]]] = P((tokenValidInsideCurlyBrackets.rep./).rep(sep = ",")./)

  def curlyBrackets[_: P]: P[Token] = P(("{" ~/ commaSepTokens ~/ ("}" | error("'}' missing"))).map(a => CurlyBrackets(a)))

  def token[_: P]: P[Token] = P((tokenValidInsideCurlyBrackets | comma | curlyBrackets)./)
  ...
  def rangeInsideSquareBrackets[_: P]: P[Range] = P((anyButClosingSquare ~/ (("-" ~ anyButClosingSquare./).?)).map(p => Range(p._1, p._2)))

  def ranges[_: P]: P[Seq[Range]] = P((firstRangeInsideSquareBrackets ~/ (rangeInsideSquareBrackets.rep)).map(p => p._1 :: (p._2.toList)))
